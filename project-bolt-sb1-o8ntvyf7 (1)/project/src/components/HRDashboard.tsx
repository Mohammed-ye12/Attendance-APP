...
export function HRDashboard({ entries, employees, userType }: Props) {
  ...
  // Determine if user has HR privileges (either HR or HR-ENG)
  const hasHRPrivileges = userType.startsWith('hr-') || userType.startsWith('HR-ENG-');
  
  // Filter entries based on user type
  const filteredEntries = entries.filter((entry) => {
    // Basic filters that apply to all user types
    ...
    if (userType.startsWith('HR-ENG-')) {
      // HR-ENG can only see Engineering department
      return matchesDate && matchesEmployee && matchesShift && 
             employees[entry.employeeId]?.department === 'Engineering' && matchesSection;
    } else if (userType.startsWith('op-logistic-')) {
      // Op-Logistic can only see Operations and Logistics departments
      const dept = employees[entry.employeeId]?.department;
      return matchesDate && matchesEmployee && matchesShift && 
             (dept === 'Operations' || dept === 'Logistics');
    }
  ...
  const getDashboardTitle = () => {
    if (userType.startsWith('hr-')) {
      return 'HR Access (Main)';
    } else if (userType.startsWith('HR-ENG-')) {
      return 'Engineering HR Dashboard';
    } else if (userType.startsWith('op-logistic-')) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border"
              disabled={userType.startsWith('HR-ENG-') || userType.startsWith('op-logistic-')}
            >
              ...
              {Object.values(employees)
                ...
                .filter(dept => {
                  // Filter departments based on user type
                  if (userType.startsWith('HR-ENG-')) {
                    return dept === 'Engineering';
                  } else if (userType.startsWith('op-logistic-')) {
                    return dept === 'Operations' || dept === 'Logistics';
                  }
                  return true; // HR can see all departments
                })
                .map((dept) => (
          ...
          {(departmentFilter === 'Engineering' || userType.startsWith('HR-ENG-')) && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Filter by Section
              </label>
