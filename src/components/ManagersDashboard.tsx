...
export function ManagersDashboard({ 
  ...
  const [localEntries, setLocalEntries] = useState<ShiftEntry[]>(entries);

  // Filter entries for the manager's section only
  const sectionEntries = localEntries.filter(entry => {
  ...
  const handleApprovalAction = async (entryId: string, approved: boolean, justification?: string) => {
    try {
      ...
      setLocalEntries(prev => prev.map(entry => 
        entry.id === entryId
          ? {
              ...
              otherRemark: justification || entry.otherRemark
            }
          : entry
      ));
      ...
      if (approved) {
        onApprove(entryId);
      } else if (onReject && justification) {
        onReject(entryId, justification);
      }
  ...
  const handleRejectionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await handleApprovalAction(rejectionForm.entryId, false, rejectionForm.justification);
      setRejectionForm(null);
      setRejectionError('');
    } catch (error) {
      setRejectionError('Failed to process rejection. Please try again.');
    }
  };
                ...
                {pendingEntries.map((entry) => (
                  <tr key={entry.id} className="bg-yellow-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleApprovalAction(entry.id, true)}
